...
until mongosh --host mongo-configsvr-1 --port 27019 --quiet --eval "db.adminCommand({ ping: 1 }).ok"; do sleep 2; done
echo "[Config RS Init] Config server 1 is ready"
until mongosh --host mongo-configsvr-2 --port 27019 --quiet --eval "db.adminCommand({ ping: 1 }).ok"; do sleep 2; done
echo "[Config RS Init] Config server 2 is ready"
until mongosh --host mongo-configsvr-3 --port 27019 --quiet --eval "db.adminCommand({ ping: 1 }).ok"; do sleep 2; done
echo "[Config RS Init] Config server 3 is ready"

# Initialize Config Server Replica Set
echo "[Config RS Init] Initializing config server replica set 'cfgRS'..."
mongosh --host mongo-configsvr-1 --port 27019 --quiet --eval "
  try {
    rs.status();
